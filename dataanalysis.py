...
def clean_up_datafile(filepath,filepathpost):
    ...
    np.savetxt(filepathpost, postdata, delimiter="\t", fmt="%10.5f", header=header)

def plots(filepathpost):

    data = np.loadtxt(filepathpost, delimiter="\t",skiprows=1).T
    #Definir eje de omegas y R
    omega_unique = np.unique(data[0])
    r_unique = np.unique(data[1])
    #Definir variables: Omega, R y Potencia
    omega = data[2].reshape(omega_unique.shape[0], r_unique.shape[0])
    ...
    #Graficas de curvas de potencia 2D
    for i in range(omega.shape[1]): 
        fig.add_trace(go.Scatter(name='Omega {}'.format(i),x=omega[:,i], y=P[:,i]), row=1,col=2)
    for i in range(r.shape[0]):
        fig.add_trace(go.Scatter(name='Resistance {}'.format(i),x=r[i], y=P[i]), row=2,col=2)

    #Graficas de Potencia y Tnesion, Tension y corriente
    ...
    fig.update_xaxes(title_text= "Omega [RPM]", row=1, col=2)
    fig.update_xaxes(title_text= "Resistance[Ohm]", row=2, col=2)
...
def mainfunc(filepathRAW,filepathPost):
    clean_up_datafile(filepathRAW,filepathPost)
    plots(filepathPost)

dirRaw=r"C:\Users\Mau\OneDrive - Estudiantes ITCR\Escritorio\TEC\Semestre 11\Asistencia LIENE\LabEolicaControl-main\AAA.txt"
dirPost=r"C:\Users\Mau\OneDrive - Estudiantes ITCR\Escritorio\TEC\Semestre 11\Asistencia LIENE\LabEolicaControl-main\postdata.txt"


mainfunc(dirRaw,dirPost)